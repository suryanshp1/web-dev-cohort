...
def main() -> None:
    ...
    tf_parser.add_argument("term", type=str, help="Search terms to search counts for")

    idf_parser = subparsers.add_parser(
        "idf", help="Calculate inverse document frequency"
    )
    idf_parser.add_argument("term", type=str, help="Search terms to search counts for")
    ...
    tfidf_parser.add_argument(
        "term", type=str, help="Search terms to search counts for"
    )
    ...
    match args.command:
        ...
        case "bm25search":
            ...
            for idx, res in enumerate(bm25_results, start=1):
                print(
                    f"{idx}. ({res.get("doc_id")}) {res.get("title")} - Score: {res.get("score", ""):.2f}"
                )
        case _:
            parser.print_help()
